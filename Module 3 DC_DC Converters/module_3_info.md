...
The Buck converter operates using a PWM signal applied to the power switch. This PWM signal is typically generated by comparing a sawtooth waveform (Vr) with a control voltage level (Vcr), as shown in the left side of the figure below. When Vr is less than Vcr, the gate signal (Vg) is high, turning the switch ON. When Vr exceeds Vcr, the gate signal becomes low, turning the switch OFF.
...
As shown in the right side of the figure above, the Buck converter operates in two possible current modes:

1. **Continuous Conduction Mode (CCM):** The inductor current never falls to zero during the switching cycle. The current waveform has a triangular ripple superimposed on a DC level. During the ON period (kT), the current ramps up with a slope of (Vin-Vout)/L. During the OFF period ((1-k)T), it ramps down with a slope of -Vout/L.

2. **Discontinuous Conduction Mode (DCM):** The inductor current falls to zero during part of the switching cycle. This typically occurs at light loads or with small inductor values. In this mode, the current starts at zero, rises during the ON period, and then falls back to zero before the end of the switching cycle.

The mode of operation significantly affects the converter's voltage regulation characteristics and dynamic response.
